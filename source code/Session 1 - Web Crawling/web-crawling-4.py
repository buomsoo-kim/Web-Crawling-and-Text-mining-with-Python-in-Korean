...
# 영화 <라라랜드> 리뷰 첫 번째 페이지에서 총 페이지 개수 계산하기
...
t = t[:-1].replace(',', '') # 앞에서 받은 숫자에 있는 콤마를 제거해야 integer 전환시 에러가 없음
                            ...
                            # 즉 예를 들어 (1,091)에서 양 괄호를 지워야 하는데 우선 오른쪽 괄호만 
                            # 지우기 위해 t[:-1]로 처리함
...
# txt 파일로 리뷰 내용 출력하기
file = open('data-full.txt', 'w', encoding = 'utf-8')
for review in review_list:
	file.write(review + '\n') # 각 리뷰를 줄 단위로 txt 파일에 저장
file.close()

# ///// END OF web-crawling-4 /////////////////////////////////////////////////